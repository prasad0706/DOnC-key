...
5. **Test Document Upload:**
   ...
   - Select a PDF, image, DOCX, XLSX, or CSV file
   - Click "Upload" and observe the upload progress bar
   - Verify the document goes to "queued" status and then "ready" (if background worker and Redis are running)

6. **Test API Key Generation:**
   - Navigate to the API Keys section on the Projects or Dashboard page
   - Click "Generate API Key"
   - Verify the modal appears with a newly generated, token-authenticated key
   - Test the "Copy to Clipboard" functionality

7. **Test Usage Analytics:**
   - Navigate to the Usage page
   - Verify all metrics (API volume, latency, errors) display real database aggregation stats correctly
...
- [ ] Document upload and parsing works (PDF, DOCX, XLSX, CSV, Images)
- [ ] API key generation and copying works
...
- The frontend is fully integrated with active backend REST API endpoints. Ensure that MongoDB, Redis, and the BullMQ background workers are running to test the complete pipeline.
